...
export default function BulkReceiveDialog({ open, onOpenChange, banks, selectedIds, selectedTotal, onSuccess }: Props) {
    ...
    const canSubmit = useMemo(
        () => Boolean(contaBancariaId) && selectedIds.length > 0 && dataRecebimento,
        [contaBancariaId, dataRecebimento, selectedIds.length]
    )
    ...
    async function handleSubmit() {
        ...
        setSubmitting(true)
        try {
            const response = await fetch("/api/financeiro/receivables/bulk-receive", {
                method: "POST",
            ...
            if (!response.ok) {
                ...
                throw new Error(body.error || "Falha ao registrar recebimentos")
            }

            toast.success("Recebimentos em lote processados")
            onOpenChange(false)
            await onSuccess()
        } catch (error) {
            ...
            <DialogContent className="border-[#2C201B]/10 bg-[#FFFCF7] sm:max-w-[460px]">
                <DialogHeader>
                    ...
                    <DialogDescription className="text-[#2C201B]/65">
                        Registre o recebimento integral das contas selecionadas em uma única operação.
                    </DialogDescription>
                </DialogHeader>
                ...
                <DialogFooter className="gap-2">
                    ...
                    <Button type="button" className="btn-primary" onClick={handleSubmit} disabled={!canSubmit || submitting}>
                        {submitting ? <><Loader2 className="mr-2 size-4 animate-spin" /> Processando...</> : "Confirmar recebimentos"}
                    </Button>
                </DialogFooter>
            </DialogContent>
        </Dialog>
