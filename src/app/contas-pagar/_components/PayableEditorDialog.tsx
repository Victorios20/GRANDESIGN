...
interface Props {
    ...
    item?: PayableListItem | null
    categories: CategoryOption[]
    centrosCusto: CentroCustoOption[]
    suppliers: SupplierOption[]
    onSuccess: () => Promise<void> | void
    onRequestPay: (item: PayableListItem) => void
}
...
export default function PayableEditorDialog({
    open,
    onOpenChange,
    item,
    categories,
    ...
    useEffect(() => {
        ...
        setFornecedorId(item?.fornecedor?.id ? String(item.fornecedor.id) : "none")
    ...
    const supplierItems = useMemo(
        () => [{ value: "none", label: "Sem fornecedor" }, ...suppliers.map((supplier) => ({ value: String(supplier.id), label: supplier.nome }))],
        [suppliers]
    )
        ...
        try {
            ...
            const payload = isInstallmentCreation
                ? {
                    ...
                    fornecedor_id: fornecedorId === "none" ? null : Number(fornecedorId),
            ...
            const url = isEdit
                ? `/api/financeiro/payables/${item!.id}`
                : isInstallmentCreation
                    ? "/api/financeiro/payables/create-installments"
                    : "/api/financeiro/payables/create"
                        ...
                        {item?.pedido_compra ? (
                            <div className="rounded-lg border border-[#ddd7cc] bg-[#faf8f4] px-3 py-2 text-sm text-[#6f6556]">
                                ...
                                <Link
                                    ...
                                    className="font-medium text-[#393316] underline-offset-2 hover:underline"
                                    onClick={() => onOpenChange(false)}
                                >
                                    {formatPedidoId(item.pedido_compra.id, item.pedido_compra.obra_id)}
                                </Link>
                                . Alterações financeiras válidas continuam sincronizadas com o pedido.
                            </div>
                        ) : null}
