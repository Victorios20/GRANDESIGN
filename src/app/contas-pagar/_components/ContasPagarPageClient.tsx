...
import PayableEditorDialog from "./PayableEditorDialog"
...
export default function ContasPagarPageClient({
    initialData,
    initialSummary,
    banks,
    categories,
    ...
    const fetchData = useCallback(async (targetPage: number) => {
        ...
        try {
            ...
            if (!listResponse.ok || !summaryResponse.ok) throw new Error("Falha ao carregar contas a pagar")
            ...
            const message = (requestError as Error).message || "Falha ao carregar contas a pagar"
            setError(message)
        } finally {
            setLoading(false)
        }
    ...
    useEffect(() => {
        ...
        fetch(`/api/financeiro/payables/${itemId}`)
            .then((res) => res.json())
            .then((item) => {
                 if (item) {
    ...
    function openEditDialog(item: PayableListItem) {
        setSelectedItem(item)
        setEditorOpen(true)
    }

    function openPaymentModal(item: PayableListItem) {
        setSelectedItem(item)
    ...
    async function handlePaymentSuccess() {
        ...
        setEditorOpen(false)
        await handleMutationSuccess("Pagamento registrado")
    }
    ...
    const payableRows = useMemo(() => data.filter((item) => canPay(item.status)), [data])
    const allSelected = payableRows.length > 0 && payableRows.every((item) => selectedIds.includes(item.id))
    const selectedItems = useMemo(() => data.filter((item) => selectedIds.includes(item.id) && canPay(item.status)), [data, selectedIds])
    const selectedTotal = useMemo(
        () => selectedItems.reduce((total, item) => total + remaining(item.valor_total, item.valor_pago), 0),
        [selectedItems]
    )
    ...
    return (
        <PageLayout title="Contas a Pagar" links={[{ label: "Home", href: "/" }]} pageBackground="bg-[#F7F4EE]">
            <div className="space-y-4">
                <section className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                    <div className="space-y-1">
                        <h1 className="text-xl font-bold tracking-tight text-[#393316] md:text-2xl">Contas a Pagar</h1>
                        ...
                        <div className="flex flex-col gap-3 xl:flex-row xl:items-center">
                            <div className="relative min-w-0 flex-1">
                                ...
                                <Input
                                    ...
                                    placeholder="Buscar por ID, descrição ou fornecedor"
                            ...
                            <thead className={operationalListTableHeadClass}>
                                <tr className={operationalListTableHeadRowClass}>
                                    <th className={cn(operationalListTableHeadCellClass, "w-12")}>
                                        <Checkbox
                                            checked={allSelected}
                                            onCheckedChange={(checked) => setSelectedIds(checked ? payableRows.map((item) => item.id) : [])}
                                                        ...
                                                        <DropdownMenuContent align="end">
                                                            ...
                                                            {item.pedido_compra ? (
                                                                <DropdownMenuItem asChild>
                                                                    <Link href={`/pedido_compra/ver/${item.pedido_compra.id}`} onClick={(event) => event.stopPropagation()}>
                                                                        Abrir pedido de compra
                                                                    </Link>
                                                                </DropdownMenuItem>
                                                            ) : null}
                                                            {canPay(item.status) ? (
                                                                <DropdownMenuItem onClick={() => openPaymentModal(item)}>Registrar pagamento</DropdownMenuItem>
                                                            ) : null}
                                                        </DropdownMenuContent>
                                                    </DropdownMenu>
                                                </td>
            ...
            <BulkRescheduleDialog
                ...
                endpoint="/api/financeiro/payables/bulk-reschedule"
                selectedIds={selectedItems.map((item) => item.id)}
            ...
            <BulkDeleteDialog
                ...
                endpoint="/api/financeiro/payables/bulk-delete"
                selectedIds={selectedItems.map((item) => item.id)}
                selectedCount={selectedItems.length}
                title="Excluir contas selecionadas"
                description="Use essa ação apenas para lançamentos abertos sem baixa parcial nem vínculo operacional."
