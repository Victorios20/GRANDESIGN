// src/app/api/uploads/contrato/route.ts
...
export async function POST(req: NextRequest) {
    ...
    try {
        ...
        const key = `contratos/${new Date().toISOString().slice(0, 10)}/${crypto.randomUUID()}.${ext}`
        ...
        return json(
            { ok: false, error: "UPLOAD_FAILED", message: "Falha ao enviar contrato.", detail: String(err?.message ?? err), requestId },
            500,
            requestId
        )
    }
