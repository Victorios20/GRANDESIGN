...
import {
  estornarIntegracaoFinanceiraPedido,
...
function mapErrorStatus(code: string) {
  const map: Record<string, number> = {
    ...
    PEDIDO_NAO_INTEGRADO: 409,
    PEDIDO_FINANCEIRO_INCONSISTENTE: 409,
    ESTORNO_FINANCEIRO_FALHOU: 500,
  }
...
export async function POST(_: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const data = await estornarIntegracaoFinanceiraPedido(pedidoId, actorId)
    ...
    console.error("[POST /api/pedido_compra/estornar/[id]] unexpected", error)
