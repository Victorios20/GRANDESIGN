...
import { criarPedidoCompraComItens, PedidoCompraCreateError } from "@/actions/pedido_compra/create-pedido-compra-db"
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const result = await criarPedidoCompraComItens({
    ...
    if (err instanceof PedidoCompraCreateError) {
      const map: Record<string, number> = {
        PAYLOAD_INVALIDO: 400,
        OBRA_NAO_ENCONTRADA: 404,
        FORNECEDOR_NAO_ENCONTRADO: 404,
        PEDIDO_CREATE_FAILED: 500,
        ITENS_CREATE_FAILED: 500,
        AUDIT_FAILED: 500,
      }
      const status = map[err.code] ?? 500
      return json({ error: err.message, code: err.code, step: err.step, details: err.details, requestId: requestId2 }, status, requestId2)
    }
    console.error("[POST /api/pedido_compra/cadastrar] unexpected", err)
