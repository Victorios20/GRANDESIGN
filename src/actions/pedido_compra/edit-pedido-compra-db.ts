...
export type PedidoCompraEditErrorCode =
  ...
  | "ITENS_REPLACE_FAILED"
  | "AUDIT_FAILED"

export class PedidoCompraEditError extends Error {
  code: PedidoCompraEditErrorCode
  step?: string
...
export type EditarPedidoCompraResult = {
  pedidoCompraId: number
  itensIds: number[]
}

export async function editarPedidoCompraComItens(
  input: EditarPedidoCompraInput
): Promise<EditarPedidoCompraResult> {
  if (!Number.isFinite(Number(input?.pedidoCompraId))) {
    throw new PedidoCompraEditError("PAYLOAD_INVALIDO", "pedidoCompraId inválido.", "validate", {
      field: "pedidoCompraId",
    })
  }

  if (!Number.isFinite(Number(input?.obra_id))) {
    throw new PedidoCompraEditError("PAYLOAD_INVALIDO", "obra_id inválido.", "validate", { field: "obra_id" })
  }

  const categoria = mapCategoria(input?.categoria as any)
  if (!categoria) {
    throw new PedidoCompraEditError("PAYLOAD_INVALIDO", "categoria inválida.", "validate", { field: "categoria" })
  }

  const itens = Array.isArray(input?.itens) ? input.itens : []
  if (!itens.length) {
    throw new PedidoCompraEditError("PAYLOAD_INVALIDO", "itens é obrigatório (mínimo 1).", "validate", { field: "itens" })
  }

  for (let i = 0; i < itens.length; i++) {
    const it = itens[i] as any
    if (!String(it?.descricao ?? "").trim()) {
      throw new PedidoCompraEditError("PAYLOAD_INVALIDO", "Item sem descricao.", "validate", {
        index: i,
        field: "itens.descricao",
      })
    }
    const reqNum = ["quantidade", "preco_unitario", "total"] as const
    for (const k of reqNum) {
      if (!Number.isFinite(Number(it?.[k]))) {
        throw new PedidoCompraEditError("PAYLOAD_INVALIDO", `Item com número inválido: ${k}.`, "validate", {
          index: i,
          field: `itens.${k}`,
        })
      }
    ...
    if (it?.tamanho != null && it?.tamanho !== "" && !Number.isFinite(Number(it?.tamanho))) {
      throw new PedidoCompraEditError("PAYLOAD_INVALIDO", "Item com tamanho inválido.", "validate", {
        index: i,
        field: "itens.tamanho",
      })
    }
  ...
  return await prisma.$transaction(
    async (tx) => {
      ...
      const obra = await tx.obras.findUnique({
        where: { id: Number(input.obra_id) },
        select: { id: true },
      })
      if (!obra) {
        throw new PedidoCompraEditError("OBRA_NAO_ENCONTRADA", "Obra não encontrada.", "load-obra", { obra_id: input.obra_id })
      }

      if (input.fornecedor_id != null) {
        const forn = await tx.fornecedores.findUnique({
          where: { id: Number(input.fornecedor_id) },
          select: { id: true },
        })
        if (!forn) {
          throw new PedidoCompraEditError("FORNECEDOR_NAO_ENCONTRADO", "Fornecedor não encontrado.", "load-fornecedor", {
      ...
      try {
        ...
        throw new PedidoCompraEditError("PEDIDO_UPDATE_FAILED", "Erro ao atualizar pedido_compra.", "update-pedido", {
      ...
      let itensIds: number[] = []
      try {
        await tx.pedido_itens.deleteMany({ where: { pedido_compra_id: Number(input.pedidoCompraId) } })

        const rows = await Promise.all(
          itens.map((it) =>
            tx.pedido_itens.create({
              data: {
                pedido_compra: { connect: { id: Number(input.pedidoCompraId) } },
      ...
      try {
        await tx.auditLog.create({
          data: {
            ...
            action: "PEDIDO_COMPRA_UPDATE",
            entity: "pedido_compra",
            entity_id: Number(input.pedidoCompraId),
            detail: {
