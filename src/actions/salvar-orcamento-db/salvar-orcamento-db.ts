...
export async function salvarOrcamento(params: {
  ...
  const novoCliente = await supabase.from('cliente').insert({
    nome: cliente.nome,
  ...
  const tipoObra = await supabase.from('tipo_obra').select('id').eq('tipo_obra', parametros.tipoObra).single()
  if (!tipoObra.data) throw new Error('Tipo de obra não encontrado')

  const orcamento = await supabase.from('orcamento').insert({
  ...
  const materiaisToInsert = [
    ...materiais.madeiras.map(m => ({
      ...
      total: m.tamanho ? Number(String(m.tamanho).replace(",", ".")) * m.quantidade * m.preco : 0,
      frete: 0,
    })),
