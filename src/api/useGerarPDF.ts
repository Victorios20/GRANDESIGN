...
export type GerarPDFResponse = PdfLinks[]
...
const fmt = (n: number) => (Math.ceil(n * 100) / 100).toFixed(2).replace(".", ",")

const REQUEST_TIMEOUT_MS = 60_000

export async function gerarPDF(params: GerarPDFParams): Promise<GerarPDFResponse> {
  const {
    orcamentoId,
    cliente,
    parametros,
  ...
  const ENDPOINT_GERAR_PDF = process.env.NEXT_PUBLIC_ENDPOINT_GERAR_PDF as
    | string
    | undefined
  ...
  try {
    const { data } = await axios.post<GerarPDFResponse>(ENDPOINT_GERAR_PDF, payload, {
      headers: { "Content-Type": "application/json" },
      timeout: REQUEST_TIMEOUT_MS,
    })

    console.log(
      "[useGerarPDF] telhas DINÂMICAS =>",
      Object.keys(payload.telhaValoresDinamicos),
      payload.telhaValoresDinamicos,
    )
    console.log("[DEBUG] Links capturados:", data)
